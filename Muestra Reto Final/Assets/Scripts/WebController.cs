...
public class WebController : MonoBehaviour
{
    ...
    IEnumerator SendData(string data)
    {
        ...
        string url = "https://sotero-agentes-server.us-south.cf.appdomain.cloud/reto/trafficLights";
    ...
    IEnumerator SendData2(string data)
    {
        ...
        string url = "https://sotero-agentes-server.us-south.cf.appdomain.cloud/reto/cars";
        //using (UnityWebRequest www = UnityWebRequest.Post(url, form))
        using (UnityWebRequest www = UnityWebRequest.Post(url, form))
        {
            byte[] bodyRaw = System.Text.Encoding.UTF8.GetBytes(data);
    ...
    void Update()
    {
        ...
        if (positions.Count > 1)
        {
            ...
            for (int t = 0; t < trafficLights.Length; t++)
            {
                List<int> last = trafficLightStatus[trafficLightStatus.Count-1];
                if (last[t] == 0)
                {
                    trafficLights[t].color = Color.green;

                } else if (last[t] == 1)
                {
                    trafficLights[t].color = Color.red;

                } else 
                {
                    trafficLights[t].color = Color.yellow;
            ...
            for (int s = 0; s < cars.Length; s++)
            {
                ...
                //Vector3 l = new Vector3(last[s][0], last[s][1], last[s][2]);
                //Vector3 pl = new Vector3(prevLast[s][0], prevLast[s][1], prevLast[s][2]);
                ...
                if (positions[positions.Count-1][s][3] == 0) 
                {
                    cars[s].transform.localRotation = Quaternion.Euler(0, 270, 0);

                } else if (positions[positions.Count-1][s][3] == 1)
                {
                    cars[s].transform.localRotation = Quaternion.Euler(0, 0, 0);

                } else if (positions[positions.Count-1][s][3] == 2)
                { 
                    cars[s].transform.localRotation = Quaternion.Euler(0, 90, 0);

                } else if (cars[s].transform.position.x == 2.5 || (cars[s].transform.position.x == 22.5 && (cars[s].transform.position.z <= 32.5 && cars[s].transform.position.z >= 7.5))) {
                    cars[s].transform.localRotation = Quaternion.Euler(0, -90, 0);

                } else {
                    cars[s].transform.localRotation = Quaternion.Euler(0, 180, 0);
                }
